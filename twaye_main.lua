...
do
    ...
    task.spawn(function()
        ...
        local RunService = game:GetService("RunService")
        local LP = game:GetService("Players").LocalPlayer
        while not LP do task.wait(0.2); LP = game:GetService("Players").LocalPlayer end
        ...
        local function parseGen(text)
            ...
            local s = tostring(text):gsub("%$", ""):gsub("/s", ""):gsub(",", "")
            local num = s:match("^%s*([%d%.]+)")
            if not num then return 0 end
            local n = tonumber(num) or 0
            local suf = s:upper():match("([KMBT])")
            if suf == "K" then n = n * 1e3
            elseif suf == "M" then n = n * 1e6
            elseif suf == "B" then n = n * 1e9
            elseif suf == "T" then n = n * 1e12 end
            return n
        end

        local function resolveGen(model)
            if not model then return 0, "", "Brainrot" end
            local idx = model.Name
            local mut = (model:GetAttribute("Mutation")) or nil
            local traits = nil
            -- AnimalsData/AnimalsShared are top-level locals defined elsewhere in
            -- this script; both are populated by loadModules() on startup.
            if AnimalsData and AnimalsShared and AnimalsData[idx] then
                local info = AnimalsData[idx]
                local ok, gen = pcall(function()
                    return AnimalsShared:GetGeneration(idx, mut, traits, nil)
                end)
                local display = (info and info.DisplayName) or idx
                if ok and type(gen) == "number" then
                    -- format a readable gen text like the game does ($1.5M/s)
                    local t
                    if NumberUtils and NumberUtils.ToString then
                        local okN, str = pcall(function() return NumberUtils:ToString(gen) end)
                        if okN and str then t = "$" .. str .. "/s" end
                    end
                    return gen, t or ("$" .. tostring(gen) .. "/s"), display
                end
            end
            -- Fallback: BillboardGui parse (legacy behaviour).
            return 0, "", (model and model.Name) or "Brainrot"
        end

        -- Legacy BillboardGui name+gen extractor; only used if AnimalsData lookup
        -- misses entirely (e.g. on the very first frame before modules load).
        local RARITY_WORDS = {
        ...
        local function readModelInfo(model)
            ...
            if not model then return nameFound, genFound end
                ...
                if bb:IsA("BillboardGui") then
                    for _, lbl in ipairs(bb:GetDescendants()) do
                        if lbl:IsA("TextLabel") and lbl.Text and lbl.Text ~= "" then
                            local t = lbl.Text:match("^%s*(.-)%s*$") or ""
                            local tl = t:lower()
                            if not RARITY_WORDS[tl] then
                                if t:match("^%$[%d%.]+[KkMmBbTt]?/s$") then
                                    if genFound == "" then genFound = t end
                                elseif not t:match("^%$") and not t:match("^[%d%.]+[KkMmBbTt]?$") then
        ...
        local function getScanRoot()
            ...
            for _, name in ipairs({
                "Conveyor","Esteira","ShopAnimals","Shop","AnimalShop",
                "ConveyorBelt","Belt","Animals"
            }) do
                local f = Workspace:FindFirstChild(name)
                if f then cachedRoot = f; return f end
            end
            return Workspace -- fallback to full walk if no candidate folder
        end

        -- Debug dump: prints every enabled ProximityPrompt under the scan root,
        -- whether or not it matched the "buy" text filter, so a mismatch is
        -- visible instead of the feature silently doing nothing. Callable
        -- manually as _G.MeerkoAutoBuyDebug().
        local function debugScanDump()
            local root = getScanRoot()
            local all, matched = {}, 0
            for _, obj in ipairs(root:GetDescendants()) do
                if obj:IsA("ProximityPrompt") then
                    local atxt = obj.ActionText or ""
                    local otxt = obj.ObjectText or ""
                    local hit = (atxt.."" ):lower():find("buy") or atxt:lower():find("purchase") or atxt:lower():find("comprar")
                              or atxt:lower():find("shop") or atxt:lower():find("acquire")
                              or otxt:lower():find("buy") or otxt:lower():find("purchase") or otxt:lower():find("comprar")
                    if hit then matched = matched + 1 end
                    all[#all+1] = string.format(
                        "  [%s] Enabled=%s ActionText=%q ObjectText=%q Parent=%s",
                        obj:GetFullName(), tostring(obj.Enabled), atxt, otxt,
                        obj.Parent and obj.Parent.Name or "nil"
                    )
                end
            end
            warn(("[AUTO BUY CARPET] debug: root=%s totalPrompts=%d textMatches=%d"):format(
                root:GetFullName(), #all, matched))
            for i = 1, math.min(#all, 20) do warn(all[i]) end
            if #all > 20 then warn(("  ...and %d more"):format(#all - 20)) end
        end
        ...
        -- Resolve the purchase remote from Packages.Net by name keyword.
        ...
        local function resolvePurchaseRemote()
            ...
            pcall(function()
                local net = MK_RS:FindFirstChild("Packages") and MK_RS.Packages:FindFirstChild("Net")
        ...
        local function notify(msg)
            warn("[AUTO BUY CARPET] " .. tostring(msg))
        end

        local HOVER_HEIGHT  = 5      -- studs above the locked part
        local BUY_INTERVAL  = 0.08   -- prompt fire cadence
        local FLY_SPEED     = 350    -- carpet-grapple flight speed to first reach the brainrot

        local lockedTarget = nil   -- the {name, prompt, part, model, genText, ...}
        local hoverConn    = nil
                                   -- BodyPosition was getting flagged by anti-cheat)

        local function lockAlive()
            return lockedTarget
                and lockedTarget.part and lockedTarget.part.Parent
                and (not lockedTarget.model or lockedTarget.model.Parent)
        end
        ...
        local function clearLock(reason)
            if lockedTarget then notify("lock cleared: "..(reason or "")) end
            lockedTarget = nil
            stopHover()
        end

        local function engageHover()
            stopHover()
            local frameSkip = 0
            hoverConn = RunService.Heartbeat:Connect(function()
                if not enabled or not lockAlive() then return end
                frameSkip = frameSkip + 1
                if frameSkip < 3 then return end
                frameSkip = 0
                local char = LP.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if not hrp or not lockedTarget.part or not lockedTarget.part.Parent then return end
                local _, ry = hrp.CFrame:ToEulerAnglesYXZ()
                local target = lockedTarget.part.Position + Vector3.new(0, HOVER_HEIGHT, 0)
                pcall(function()
                    hrp.CFrame = CFrame.new(target) * CFrame.Angles(0, ry, 0)
                    hrp.AssemblyLinearVelocity = Vector3.zero
                    hrp.AssemblyAngularVelocity = Vector3.zero
                end)
            end)
        ...
        -- Pick the best qualifying target from the current scan.
        local function pickTarget()
            local pets = scanConveyorCarpet()
            if #pets == 0 then return nil end
            local char = LP.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            if not hrp then return nil end
            local minGen = tonumber(Config.AutoBuyMinGen) or 1000000
            local best, bestScore = nil, math.huge
            for _, p in ipairs(pets) do
                if p.part and p.part.Parent and p.genValue >= minGen then
                    local d = (hrp.Position - p.part.Position).Magnitude
                    if d < bestScore then best = p; bestScore = d end
                end
            end
            if not best then
                -- testing fallback: nearest with no threshold
                for _, p in ipairs(pets) do
                ...
                if best then notify("no >min, fallback nearest: "..best.name) end
            end
            return best
        end

        local _emptyPasses = 0
        local _lastDebugDump = 0
        local function buyPass()
            if busy then return end
            if lockAlive() then return end
            if lockedTarget then clearLock("target gone") end

            local best = pickTarget()
            if not best then
                _emptyPasses = _emptyPasses + 1
                -- nothing found for ~15s straight: dump why, at most once per 30s
                if _emptyPasses >= 10 and os.clock() - _lastDebugDump > 30 then
                    _lastDebugDump = os.clock()
                    notify("nothing buyable found after " .. _emptyPasses .. " tries, dumping scan:")
                    pcall(debugScanDump)
                end
                return
            end
            ...
            busy = true
            task.spawn(function()
                notify(("Locked: %s (%s)"):format(best.name, best.genText ~= "" and best.genText or "no-gen"))

                -- Equip the grapple/carpet so we have flight state (per user spec).
                pcall(function() if carpetEngage then carpetEngage() end end)
                task.wait(0.1)

                local char = LP.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if not hrp or not best.part or not best.part.Parent then
                    notify("no char / target gone")
                    clearLock("no char")
                    busy = false
                    return
                end

                local snapTo = best.part.Position + Vector3.new(0, HOVER_HEIGHT, 0)
                pcall(function()
                    hrp.CFrame = CFrame.new(snapTo)
                    hrp.AssemblyLinearVelocity = Vector3.zero
                    hrp.AssemblyAngularVelocity = Vector3.zero
                end)
                notify("snapped, hovering + buying...")

                -- Engage the BodyPosition tether (rides the conveyor as it moves).
                engageHover()
        ...
        task.spawn(function()
            while true do
                task.wait(1.5)
                if enabled and not busy then pcall(buyPass) end
            end
        end)

        _G.MeerkoAutoBuyCarpet = function(state)
            enabled = state and true or false
            Config.AutoBuyCarpet = enabled
            if SaveConfig then pcall(SaveConfig) end
        end

        if Config.AutoBuyCarpet then enabled = true end
    end)
