...
local function vizPath(fromPos, waypoints)
    ...
    for _, wp in ipairs(waypoints) do
        vizLine(prev, wp, Color3.fromRGB(215, 165, 0))
...
-- Floor 1 is ground-only: suppress any vertical velocity so the mover never
-- lifts or sinks the character while travelling between waypoints.
...
local function computeRoute(fromPos, toPos, facingDir)
    ...
    -- Floor 1 is ground-only: never arc upward or add float offsets.
    -- Only use lift routes / float offsets when travelling to/from upper floors.
    ...
    -- On floor 1 keep waypoints at ground level (FLOAT = 0); upper floors get
    -- a small float to avoid clipping through geometry.
    ...
    if ok and path.Status == Enum.PathStatus.Success then
        ...
        for _, wp in ipairs(path:GetWaypoints()) do
            if (wp.Position - last).Magnitude >= 8 then
                -- On floor 1 clamp waypoint Y to the starting ground Y so
                -- navmesh glitches never push the path upward.
...
_G._enableAntiDie = _enableAntiDie
...
task.spawn(function() task.wait(3); clearViz() end)
    ...
    pcall(function()
        ...
        if _hum and not _G._floatActive then pcall(function() _hum:UnequipTools() end) end
    end)
    pcall(function()
...
local _preHoldPrompt   = nil   -- prompt dont le hold a déjà démarré
local _preHoldStartAt  = 0    -- tick() au moment où le hold a démarré
local _preHoldAdjusted = 1.3  -- durée ajustée du hold en cours
...
local STEAL_DURATION      = 1.1  -- durée pour le TP
local STEAL_DURATION_NOTP = 1.55  -- durée pour le grab sans TP
...
local function _stealExecute(prompt)
    ...
    v1Progress = 0
    task.spawn(function()
        -- Hold events déjà firés par le pre-hold dans Heartbeat, on fire trigger directement
        if not (prompt == _preHoldPrompt and _preHoldStartAt > 0) then
            -- Pas de pre-hold : fire hold puis attendre un minimum avant trigger
            for _, f in ipairs(data.hold) do task.spawn(function() pcall(f) end) end
            local _ht = STEAL_DURATION
            pcall(function()
                if prompt.HoldDuration and prompt.HoldDuration > 0 then _ht = prompt.HoldDuration end
            end)
            local _pg = _stealLocalPlayer:GetNetworkPing()
            local _adj = _ht - math.clamp(_pg * 0.75, 0, 0.10)
            local _t0b = tick()
            repeat
                v1Progress = math.clamp((tick() - _t0b) / _adj, 0, 0.99)
                _stealServices.RunService.Heartbeat:Wait()
            until tick() - _t0b >= _adj
        end
        -- Fire trigger via fireproximityprompt
...
_stealServices.RunService.Heartbeat:Connect(function()
    ...
    if StealConfig.StealNearest and not _stealManualUID then
        ...
        if target then
            ...
            -- range check supprimé : TP gère la proximité
        end
    end
    if prompt then
        if prompt ~= _preHoldPrompt then
            -- Nouveau prompt détecté : fire le hold immédiatement
            _preHoldPrompt = prompt
            if not _stealDataCache[prompt] then
                local entry = { hold = {}, trigger = {}, ready = true }
                if getconnections then
                    pcall(function()
            ...
            local _preData = _stealDataCache[prompt]
            if _preData then
                for _, f in ipairs(_preData.hold) do task.spawn(function() pcall(f) end) end
            end
            -- Calculer la durée ajustée et noter le départ
            local _ht = STEAL_DURATION
            pcall(function()
                if prompt.HoldDuration and prompt.HoldDuration > 0 then _ht = prompt.HoldDuration end
            end)
            local _pg = _stealLocalPlayer:GetNetworkPing()
            _preHoldAdjusted = _ht - math.clamp(_pg * 0.75, 0, 0.10)
            _preHoldStartAt  = tick()
        end
        -- Animer la barre pendant le pre-hold
        v1Progress = math.clamp((tick() - _preHoldStartAt) / _preHoldAdjusted, 0, 0.99)
        -- Appeler _stealExecute quand le hold est écoulé
        if tick() - _preHoldStartAt >= _preHoldAdjusted then
            _stealExecute(prompt)
        end
    else
        -- Hors range : garder le pre-hold actif et fire dès que le hold est écoulé
        if _preHoldPrompt and _preHoldStartAt > 0 then
            if _preHoldPrompt.Parent == nil then
                -- Prompt détruit : reset
                _preHoldPrompt  = nil
                _preHoldStartAt = 0
            elseif tick() - _preHoldStartAt >= _preHoldAdjusted then
                -- Hold écoulé même hors range : fire quand même
                _stealExecute(_preHoldPrompt)
            end
            -- Sinon : hold pas encore fini, on attend sans reset
        end
    end
end)
...
-- ═══ HAZE Instant Fire Layer ═══
-- Fire fireproximityprompt 25x dès qu'un prompt Steal est dans les 12 studs
task.spawn(function() pcall(function()
    local _LP = game:GetService("Players").LocalPlayer
    local _trackedH = {}
    local _lastFireH = {}
    local HAZE_RADIUS = 12
    local HAZE_BURST  = 25

    local function _hazeFirePrompt(prompt)
        local t = os.clock()
        if _lastFireH[prompt] and (t - _lastFireH[prompt]) < 0.08 then return end
        _lastFireH[prompt] = t
        for _ = 1, HAZE_BURST do
            pcall(function() fireproximityprompt(prompt, 0) end)
        end
    end

    local function _hazeGetHRP()
        local c = _LP.Character
        return c and c:FindFirstChild("HumanoidRootPart")
    end

    local function _hazeTrack(prompt)
        if _trackedH[prompt] then return end
        _trackedH[prompt] = true
        prompt:GetPropertyChangedSignal("Enabled"):Connect(function()
            if not prompt.Enabled then return end
            local hrp = _hazeGetHRP(); if not hrp then return end
            local p = prompt.Parent
            if p and p:IsA("Attachment") then p = p.Parent end
            if not p or not p:IsA("BasePart") then return end
            if (p.Position - hrp.Position).Magnitude <= HAZE_RADIUS then
                _hazeFirePrompt(prompt)
            end
        end)
        prompt.AncestryChanged:Connect(function()
            if not prompt:IsDescendantOf(workspace) then
                _trackedH[prompt] = nil
                _lastFireH[prompt] = nil
            end
        end)
    end

    local function _hazeScan()
        local plots = workspace:FindFirstChild("Plots"); if not plots then return end
        for _, plot in ipairs(plots:GetChildren()) do
            local pods = plot:FindFirstChild("AnimalPodiums"); if not pods then continue end
            for _, obj in ipairs(pods:GetDescendants()) do
                if obj:IsA("ProximityPrompt") then _hazeTrack(obj) end
            end
        end
    end
    ...
    -- Backup loop : fire toutes les 0.08s si en range
    while true do
        task.wait(0.08)
        local hrp = _hazeGetHRP(); if not hrp then continue end
        for prompt in pairs(_trackedH) do
...
-- Cache des frames admin panel pour éviter de les re-chercher à chaque appel
local _apFramesCache = { cf = nil, pf = nil }

local function _apGetFrames()
    local ap = PlayerGui:FindFirstChild("AdminPanel")
    if not ap then _apFramesCache.cf = nil; _apFramesCache.pf = nil; return nil, nil end
    ...
    local cf = c:FindFirstChild("ScrollingFrame")
    local pf = p:FindFirstChild("ScrollingFrame")
    _apFramesCache.cf = cf; _apFramesCache.pf = pf
    return cf, pf
end

local function _apFireFns(fns)
    for _, fn in ipairs(fns) do task.spawn(fn) end
end

-- Cache getconnections par bouton
local _apCommandCache = {}
local _apProfileCache = {}

local function _apCacheBtn(btn)
    local out = {}
    if getconnections then
...
local function adminpanel(player, command)
    local cf, pf = _apFramesCache.cf, _apFramesCache.pf
    if not cf or not cf.Parent then cf, pf = _apGetFrames() end
    if not cf or not pf then return false end
    local command_ui = cf:FindFirstChild(command)
    if not command_ui then return false end
    local player_ui  = pf:FindFirstChild(player.Name)
    if not player_ui then return false end
    if firesignal then
        pcall(firesignal, command_ui.MouseButton1Click)
        pcall(firesignal, command_ui.MouseButton1Down)
        pcall(firesignal, command_ui.Activated)
        pcall(firesignal, player_ui.MouseButton1Click)
        pcall(firesignal, player_ui.MouseButton1Down)
        pcall(firesignal, player_ui.Activated)
    end
    return true
end
_G._adminpanel = adminpanel

Players.PlayerRemoving:Connect(function(p) _apProfileCache[p.Name] = nil end)
...
local function runCommands(targetPlayer)
    ...
    task.spawn(function()
        ...
        -- Pré-cache tous les boutons avant de fire (comme HUB PVP)
        local cf, pf = _apGetFrames()
        if cf and pf then
            local pb = pf:FindFirstChild(targetPlayer.Name)
            if pb and not _apProfileCache[targetPlayer.Name] then
                _apProfileCache[targetPlayer.Name] = _apCacheBtn(pb)
            end
            for _, cmd in ipairs(cmdsToRun) do
                if not _apCommandCache[cmd] then
                    local cb = cf:FindFirstChild(cmd)
                    if cb then _apCommandCache[cmd] = _apCacheBtn(cb) end
                end
            end
        end
        -- Lancer toutes les commandes en parallèle (chacune dans son task.spawn)
        for _, cmd in ipairs(cmdsToRun) do
            local _cmd = cmd
            LastCommandUse[_cmd] = tick()
            if _G.ClickAP_LastUse then _G.ClickAP_LastUse[_cmd] = tick() end
            task.spawn(function()
                pcall(_G._adminpanel, targetPlayer, _cmd)
                pcall(function()
                    if _G.ShowOutgoingCmd and targetPlayer then
                        _G.ShowOutgoingCmd(targetPlayer.Name, _cmd)
                    end
                end)
            end)
        end
...
local function loadSavedKeybinds()
	...
	loadKey("SavedResetKey",      "ResetKeybind",       "X")
end
loadSavedKeybinds()
...
--[[ task.defer(function()
    pcall(function()
        ...
        local _spVal         = 16
        local _spConn        = nil
        local _spCharConn    = nil
        local _spAutoEnabled = false
        local SPEED_MIN, SPEED_MAX = 5, 23

        local pill, dot, spSet_ref

        local function setPillVisual(on)
            if not pill or not dot then return end
            pill.BackgroundColor3 = on and Color3.fromRGB(65, 195, 90) or Color3.fromRGB(55, 55, 80)
            dot.Position = on and UDim2.new(1, -21, 0.5, -9) or UDim2.new(0, 3, 0.5, -9)
        end

        local function _setWalkSpeed(val)
            pcall(function()
                local char = LocalPlayer.Character
                local hum  = char and char:FindFirstChildOfClass("Humanoid")
        ...
        local function applySpeed(on)
            ...
            if _spCharConn then _spCharConn:Disconnect(); _spCharConn = nil end
            _spOn = on
            setPillVisual(on)
            if spSet_ref then spSet_ref(on) end
            if not on then
                _setWalkSpeed(16)
                return
            end
            _setWalkSpeed(_spVal)
            -- Heartbeat pour contrer les resets du jeu
            _spConn = RunService.Heartbeat:Connect(function()
                local char = LocalPlayer.Character
                local hum  = char and char:FindFirstChildOfClass("Humanoid")
                if hum and hum.WalkSpeed ~= _spVal then hum.WalkSpeed = _spVal end
            end)
            -- Ré-appliquer après respawn
            _spCharConn = LocalPlayer.CharacterAdded:Connect(function(c)
                task.wait(0.5)
                if _spOn then
                    local hum = c:WaitForChild("Humanoid", 5)
                    if hum then hum.WalkSpeed = _spVal end
                end
            end)
        end

        -- Parent au ScreenGui du hub (TwayveHub) qui fonctionne déjà
        local _spGuiParent = nil
        pcall(function()
            local hubFrame = _G.TwayveHubFrame
            if hubFrame and hubFrame.Parent and hubFrame.Parent:IsA("ScreenGui") then
                _spGuiParent = hubFrame.Parent
            end
        end)
        if not _spGuiParent then
            local sg = Instance.new("ScreenGui")
            sg.Name = "SpeedBoostPanel"; sg.ResetOnSpawn = false
            sg.IgnoreGuiInset = true; sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
            sg.DisplayOrder = 200
            local _ok = pcall(function() sg.Parent = game:GetService("CoreGui") end)
            if not _ok or not sg.Parent then pcall(function() if gethui then sg.Parent = gethui() end end) end
            if not sg.Parent then sg.Parent = LocalPlayer:WaitForChild("PlayerGui") end
            if protect_gui then pcall(function() protect_gui(sg) end) end
            _spGuiParent = sg
        end
        ...
        win.Position               = UDim2.new(0, 10, 1, -(H + 10))
        win.BackgroundColor3       = Color3.fromRGB(14, 14, 26)
        win.BackgroundTransparency = 0.08
        win.BorderSizePixel        = 0
        win.ZIndex                 = 200
        win.Active                 = false
        win.Visible                = false
        win.Parent                 = _spGuiParent
        Instance.new("UICorner", win).CornerRadius = UDim.new(0, 10)
        do local s = Instance.new("UIStroke", win); s.Color = Color3.fromRGB(65, 65, 105); s.Thickness = 1 end

        spSet_ref = nil

        local hdr = Instance.new("Frame")
        hdr.Size             = UDim2.new(1, 0, 0, 28)
        hdr.BackgroundColor3 = Color3.fromRGB(24, 24, 44)
        ...
        do
            local fill = Instance.new("Frame", hdr)
            fill.Size             = UDim2.new(1, 0, 0, 10)
            fill.Position         = UDim2.new(0, 0, 1, -10)
            fill.BackgroundColor3 = Color3.fromRGB(24, 24, 44)
            fill.BorderSizePixel  = 0
            fill.ZIndex           = 51
        end
        do
            ...
            ttl.Text                   = "Speed Boost"
            ttl.TextColor3             = Color3.fromRGB(240, 240, 255)
        ...
        xBtn.BackgroundColor3 = Color3.fromRGB(60, 25, 25)
        xBtn.BorderSizePixel  = 0
        xBtn.Text             = "x"
        xBtn.TextColor3       = Color3.fromRGB(220, 90, 90)
        ...
        local speedValLbl = Instance.new("TextLabel", win)
        speedValLbl.Size                   = UDim2.new(1, -20, 0, 20)
        speedValLbl.Position               = UDim2.new(0, 10, 0, 34)
        speedValLbl.BackgroundTransparency = 1
        speedValLbl.Text                   = "Speed: " .. _spVal
        speedValLbl.TextColor3             = Color3.fromRGB(200, 200, 240)
        speedValLbl.Font                   = Enum.Font.GothamBold
        speedValLbl.TextSize               = 12
        speedValLbl.TextXAlignment         = Enum.TextXAlignment.Left
        speedValLbl.ZIndex                 = 51

        local TRACK_W = W - 20
        local track = Instance.new("Frame", win)
        track.Size             = UDim2.new(0, TRACK_W, 0, 8)
        ...
        local sliderFill = Instance.new("Frame", track)
        sliderFill.Size             = UDim2.new(0, 0, 1, 0)
        sliderFill.BackgroundColor3 = Color3.fromRGB(65, 195, 90)
        ...
        local function setSliderFromVal(v)
            v = math.clamp(v, SPEED_MIN, SPEED_MAX)
            _spVal = math.floor(v)
            local pct = (_spVal - SPEED_MIN) / (SPEED_MAX - SPEED_MIN)
            sliderFill.Size  = UDim2.new(pct, 0, 1, 0)
            knob.Position    = UDim2.new(0, pct * TRACK_W - 8, 0.5, -8)
            speedValLbl.Text = "Speed: " .. _spVal
            if _spOn then _setWalkSpeed(_spVal) end
        end
        setSliderFromVal(_spVal)
        ...
        local function updateFromInput(i)
            ...
            setSliderFromVal(SPEED_MIN + (rx / as.X) * (SPEED_MAX - SPEED_MIN))
        end
        track.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 then
                sliderDragging = true; updateFromInput(i)
            end
        end)
        track.InputChanged:Connect(function(i)
            if sliderDragging and i.UserInputType == Enum.UserInputType.MouseMovement then updateFromInput(i) end
        end)
        ...
        do
            local presets = {16, 18, 20, 24}
            local bW = 36; local gap = 4
            local sx = (W - (#presets * bW + (#presets - 1) * gap)) / 2
            for i, v in ipairs(presets) do
                local pb = Instance.new("TextButton", win)
                pb.Size             = UDim2.new(0, bW, 0, 20)
                pb.Position         = UDim2.new(0, sx + (i-1)*(bW+gap), 0, 76)
                pb.BackgroundColor3 = Color3.fromRGB(32, 32, 56)
                pb.BorderSizePixel  = 0
                pb.Text             = tostring(v)
                pb.TextColor3       = Color3.fromRGB(180, 180, 220)
                pb.Font             = Enum.Font.Gotham
                pb.TextSize         = 11
                pb.ZIndex           = 52
                Instance.new("UICorner", pb).CornerRadius = UDim.new(0, 5)
                pb.MouseButton1Click:Connect(function() setSliderFromVal(v) end)
            end
        end

        do
            local lbl = Instance.new("TextLabel", win)
            lbl.Size = UDim2.new(0, 80, 0, 24); lbl.Position = UDim2.new(0, 10, 0, 104)
            lbl.BackgroundTransparency = 1; lbl.Text = "Enabled:"
            lbl.TextColor3 = Color3.fromRGB(170, 170, 205); lbl.Font = Enum.Font.Gotham
            lbl.TextSize = 12; lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.ZIndex = 51
        end
        pill = Instance.new("Frame", win)
        pill.Size = UDim2.new(0, 44, 0, 24); pill.Position = UDim2.new(1, -54, 0, 104)
        pill.BackgroundColor3 = Color3.fromRGB(55, 55, 80); pill.BorderSizePixel = 0; pill.ZIndex = 52
        Instance.new("UICorner", pill).CornerRadius = UDim.new(1, 0)
        dot = Instance.new("Frame", pill)
        dot.Size = UDim2.new(0, 18, 0, 18); dot.Position = UDim2.new(0, 3, 0.5, -9)
        dot.BackgroundColor3 = Color3.fromRGB(255, 255, 255); dot.BorderSizePixel = 0; dot.ZIndex = 53
        Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
        local togBtn = Instance.new("TextButton", pill)
        togBtn.Size = UDim2.new(1, 0, 1, 0); togBtn.BackgroundTransparency = 1
        togBtn.Text = ""; togBtn.ZIndex = 54
        togBtn.MouseButton1Click:Connect(function()
            _spAutoEnabled = false
            applySpeed(not _spOn)
        end)

        -- Toggle dans les settings (row 8) : ouvre/ferme panel ET active/désactive speed
        local ok2, spBtn, spSet = pcall(makeMainToggle, "Speed Boost", _MT_START + (_MT_ROW + _MT_GAP) * 8, false)
        if ok2 and spBtn then
            spSet_ref = spSet
            spBtn.MouseButton1Click:Connect(function()
                _spAutoEnabled = false
                local newOn = not _spOn
                applySpeed(newOn)
                win.Visible = newOn
                task.defer(function() win.Visible = newOn end)
            end)
        end

        -- Heartbeat visuel + auto-invis steal
        task.spawn(function()
            local lastInvis = false
            while true do
                task.wait(0.15)

                -- Re-forcer le visuel si speed est ON (contre tout reset externe)
                if _spOn then
                    setPillVisual(true)
                    if spSet_ref then pcall(spSet_ref, true) end
                end

                -- Auto-activation quand invisible steal est ON
                local invisNow = _G.invisibleStealEnabled == true
                if invisNow and not lastInvis then
                    if not _spOn then
                        _spAutoEnabled = true
                        applySpeed(true)
                    end
                elseif not invisNow and lastInvis then
                    if _spAutoEnabled then
                        _spAutoEnabled = false
                        applySpeed(false)
                    end
                end
...
end) --]] -- Speed Boost Panel block end (disabled)

-- ── Grab Own Base (row 8) ────────────────────────────────────────────────────
task.defer(function()
    pcall(function()
        local _gobRunning = false

        local function _gobFindMyPlots()
            local plots = Workspace:FindFirstChild("Plots")
            local mine = {}
            ...
            for _, plot in ipairs(plots:GetChildren()) do
                ...
                if sign then
                    local yb = sign:FindFirstChild("YourBase")
                    if yb and yb:IsA("BillboardGui") and yb.Enabled then
                        table.insert(mine, plot); continue
                    end
                    for _, d in ipairs(sign:GetDescendants()) do
                        if d:IsA("TextLabel") then
                            local t = d.Text:lower()
        ...
        local function _gobGetPodiumPos(podium)
            local base  = podium:FindFirstChild("Base")
            local spawn = base and base:FindFirstChild("Spawn")
            if spawn then return spawn.CFrame end
        ...
        local function _gobGrabAll()
            if _gobRunning then return end
            _gobRunning = true
            pcall(function()
                local myPlots = _gobFindMyPlots()
                for _, plot in ipairs(myPlots) do
                    ...
                    for _, podium in ipairs(podiums:GetChildren()) do
                        ...
                        if #prompts == 0 then continue end
                        -- TP near the podium
                        local cf = _gobGetPodiumPos(podium)
                        if cf then
                            local char = LocalPlayer.Character
                            local hrp  = char and char:FindFirstChild("HumanoidRootPart")
                            if hrp then
                                pcall(function()
                                    hrp.CFrame = cf * CFrame.new(0, 3, 3)
                                end)
                                task.wait(0.08)
                            end
                        end
        ...
        local ok9, gobBtn, gobSet = pcall(makeMainToggle, "Grab Own Base",
            _MT_START + (_MT_ROW + _MT_GAP) * 8, false)
        if ok9 and gobBtn then
            -- Popup UI
            local _gobPopup = nil
            local _gobActive = false
            local function _closeGobPopup(keepActive)
                if _gobPopup and _gobPopup.Parent then _gobPopup:Destroy() end
                _gobPopup = nil
                if not keepActive then _gobActive = false; gobSet(false) end
            end
            local function _openGobPopup()
                -- Si déjà actif : désactiver
                if _gobActive then _closeGobPopup(false); return end
                if _gobPopup then _closeGobPopup(false); return end
                gobSet(true); _gobActive = true
                local sg = Instance.new("ScreenGui")
                sg.Name = "GobPopup"; sg.ResetOnSpawn = false; sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
                local _sgOk = pcall(function() sg.Parent = game:GetService("CoreGui") end)
                if not _sgOk or not sg.Parent then
                    pcall(function() sg.Parent = gethui and gethui() or game:GetService("Players").LocalPlayer.PlayerGui end)
                end
                if protect_gui then pcall(function() protect_gui(sg) end) end
                _gobPopup = sg

                local pop = Instance.new("Frame", sg)
                pop.Size = UDim2.new(0, 160, 0, 56)
                pop.AnchorPoint = Vector2.new(0.5, 0.5)
                pop.Position = UDim2.new(0.5, 0, 0.5, 0)
                pop.BackgroundColor3 = Color3.fromRGB(8, 10, 18)
                pop.BackgroundTransparency = 0.25
                ...
                if _G._FH_AddThemeStroke then _G._FH_AddThemeStroke(popStroke)
                else popStroke.Color = _G._FH_AccentA or Color3.fromRGB(60, 210, 100) end

                -- Étoiles flottantes (même pattern que les autres panels)
                local _starData = {{0.12,0.30,2,0.0048},{0.45,0.65,3,0.0055},{0.78,0.20,2,0.0042},{0.33,0.80,3,0.0051},{0.66,0.45,2,0.0039},{0.90,0.70,3,0.0060},{0.22,0.55,2,0.0044}}
                local _starF, _starO = {}, {}
                for i, sp in ipairs(_starData) do
                    local s = Instance.new("Frame", pop)
                    s.Size = UDim2.new(0, sp[3], 0, sp[3])
                    s.Position = UDim2.new(sp[1], 0, sp[2], 0)
                    s.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                    s.BackgroundTransparency = 0.3; s.BorderSizePixel = 0; s.ZIndex = 10
                    Instance.new("UICorner", s).CornerRadius = UDim.new(1, 0)
                    _starF[i] = s; _starO[i] = sp[1]
                end
                game:GetService("RunService").Heartbeat:Connect(function(dt)
                    for i, s in ipairs(_starF) do
                        if not s or not s.Parent then continue end
                        _starO[i] = (_starO[i] - _starData[i][4] * dt) % 1
                        s.Position = UDim2.new(_starO[i], 0, _starData[i][2], 0)
                        s.BackgroundTransparency = 0.3 + math.abs(math.sin(tick() * 0.8 + i * 0.7)) * 0.5
                    end
                end)

                -- Bouton GRAB
                local grabBtn = Instance.new("TextButton", pop)
                grabBtn.Size = UDim2.new(1, -16, 0, 30)
                grabBtn.Position = UDim2.new(0, 8, 0, 13)
                grabBtn.BackgroundColor3 = _G._FH_AccentA or Color3.fromRGB(60, 210, 100)
                ...
                Instance.new("UICorner", grabBtn).CornerRadius = UDim.new(0, 7)
                if _G._FH_AddThemeFill then _G._FH_AddThemeFill(grabBtn)
                else grabBtn.BackgroundColor3 = _G._FH_AccentA or Color3.fromRGB(60, 210, 100) end

                grabBtn.MouseButton1Click:Connect(function()
                    task.spawn(function() _gobGrabAll() end)
                end)

                -- Drag
                local _dragging, _dragStart, _popStart = false, nil, nil
                pop.InputBegan:Connect(function(inp)
                    if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
                        _dragging = true
                        _dragStart = inp.Position
                        _popStart = pop.Position
                ...
                game:GetService("UserInputService").InputChanged:Connect(function(inp)
                    if _dragging and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
                        ...
                        pop.Position = UDim2.new(
                            _popStart.X.Scale, _popStart.X.Offset + delta.X,
                            _popStart.Y.Scale, _popStart.Y.Offset + delta.Y
                        )
                    end
                end)
            end

            gobBtn.MouseButton1Click:Connect(_openGobPopup)
        end
    end)
end)

-- ── Game Stretcher (row 9) ───────────────────────────────────────────────────
task.defer(function()
    pcall(function()
        local _gsEnabled   = false
        local STRETCH_NAME = "FH_GameStretch3"
        local STRETCH_MAT  = CFrame.new(0, 0, 0, 1, 0, 0, 0, 0.8, 0, 0, 0, 1)
        local RS = game:GetService("RunService")

        local function enableStretcher()
            if _gsEnabled then return end
            _gsEnabled = true
            pcall(function() RS:UnbindFromRenderStep(STRETCH_NAME) end)
            pcall(function()
                RS:BindToRenderStep(STRETCH_NAME, 2001, function()
                    local cam = workspace.CurrentCamera
                    if cam then cam.CFrame = cam.CFrame * STRETCH_MAT end
                end)
            end)
        end

        local function disableStretcher()
            if not _gsEnabled then return end
            _gsEnabled = false
            pcall(function() RS:UnbindFromRenderStep(STRETCH_NAME) end)
        end

        local ok9, gsBtn, gsSet = pcall(makeMainToggle, "Game Stretcher",
            _MT_START + (_MT_ROW + _MT_GAP) * 9, false)
        if ok9 and gsBtn then
            gsBtn.MouseButton1Click:Connect(function()
                ...
                gsSet(_gsEnabled)
                pcall(function() SettingsObj:SetAttribute("SavedGameStretcher", _gsEnabled) end)
            end)
            pcall(function()
                local sv = SettingsObj:GetAttribute("SavedGameStretcher")
                if sv == true then
                    enableStretcher()
                    gsSet(true)
                end
...
AntiRagdoll.forceBackpack = function()
    ...
    local gui = _arv_LP:FindFirstChild("PlayerGui")
...
AntiRagdoll.enable = function()
    ...
    local _arTick = 0
    AntiRagdoll.connections.heartbeat = _arv_RunService.Heartbeat:Connect(function(dt)
        _arTick = _arTick + dt
        ...
        local endTime = _arv_LP:GetAttribute("RagdollEndTime")
        if endTime and (endTime - workspace:GetServerTimeNow()) > 0 then
            ragdolled = true
        end
        if ragdolled then
            pcall(function() _arv_LP:SetAttribute("RagdollEndTime", workspace:GetServerTimeNow()) end)
            AntiRagdoll.removeRagdollConstraints(char)
            for _, obj in ipairs(char:GetDescendants()) do
                if obj:IsA("Motor6D") and obj.Enabled == false then
                    obj.Enabled = true
    ...
    AntiRagdoll.connections.charAdded = _arv_LP.CharacterAdded:Connect(function(char)
        task.wait(1)
        AntiRagdoll.forceBackpack()
        AntiRagdoll.onCharacterAdded_AR(char)
    end)
    if _arv_LP.Character then AntiRagdoll.onCharacterAdded_AR(_arv_LP.Character) end
    task.spawn(function()
...
-- ── Toggle: Ragdoll (Anti-Ragdoll HUB PVP) ───────────────────────────────────
local _arV2On = false
...
do
    ...
    y = y + _TP_ROW + _TP_GAP
    local _tpF2Btn, _tpF2Set = makeTpToggle("2e etage → Floor 1 (par en dessous)", y, fe.floor2AsFloor1 == true)
    y = y + _TP_ROW + _TP_GAP
...
do
    ...
    -- ── FPS Boost / Optimizations (HUB PVP exact) ────────────────────────────
    ...
    -- AnimRemove
    local _AnimRemove = { connections = {}, animators = {} }
    _AnimRemove.isLocalCharacter = function(model)
        return LocalPlayer.Character == model
    end
    _AnimRemove.isMiniFollower = function(model)
        while model do
    ...
    _AnimRemove.handleAnimator = function(animator)
        local model = animator:FindFirstAncestorOfClass("Model")
        if model and _AnimRemove.isLocalCharacter(model) then return end
        if model and game:GetService("Players"):GetPlayerFromCharacter(model) then return end
        if _AnimRemove.isMiniFollower(animator) then return end
        ...
        table.insert(_AnimRemove.connections, conn)
        table.insert(_AnimRemove.animators, animator)
    end
    _AnimRemove.enable = function()
        local _step = 0
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("Animator") then _AnimRemove.handleAnimator(obj) end
            _step = _step + 1
            if _step % 500 == 0 then task.wait() end
        end
        _AnimRemove.connections.desc = workspace.DescendantAdded:Connect(function(obj)
            if obj:IsA("Animator") then _AnimRemove.handleAnimator(obj) end
        end)
    end
    _AnimRemove.disable = function()
        for _, conn in pairs(_AnimRemove.connections) do
            if conn then conn:Disconnect() end
        end
        _AnimRemove.connections = {}
        for _, animator in ipairs(_AnimRemove.animators) do
            if animator and animator.Parent then
                animator.Enabled = false
                task.defer(function()
                    if animator and animator.Parent then animator.Enabled = true end
                end)
            end
        end
        _AnimRemove.animators = {}
    end

    -- FPS
    local _FPS = { connections = {} }
    _FPS.stripVisuals = function(obj)
    ...
    _FPS.destroyAllAccessories = function()
    ...
    local function _fpsRemoveMeshes(tool)
        if not tool:IsA("Tool") then return end
        local handle = tool:FindFirstChild("Handle")
        if not handle then return end
        for _, d in ipairs(handle:GetDescendants()) do
            if d:IsA("Mesh") or d:IsA("SpecialMesh") then pcall(function() d:Destroy() end) end
        end
    end
    _FPS.enable = function()
        local Lighting = game:GetService("Lighting")
        ...
        task.spawn(function() _FPS.destroyAllAccessories() end)
        for _, obj in ipairs(workspace:GetDescendants()) do
            _FPS.stripVisuals(obj)
        end
        _FPS.connections.descendant = workspace.DescendantAdded:Connect(function(obj)
        ...
        local function onFPSCharacter(char)
            char.ChildAdded:Connect(_fpsRemoveMeshes)
            for _, child in ipairs(char:GetChildren()) do _fpsRemoveMeshes(child) end
        end
        _FPS.connections.player = game:GetService("Players").PlayerAdded:Connect(function(plr)
            plr.CharacterAdded:Connect(onFPSCharacter)
        end)
        for _, plr in ipairs(game:GetService("Players"):GetPlayers()) do
            if plr.Character then onFPSCharacter(plr.Character) end
        end
        _FPS.connections.localChar = LocalPlayer.CharacterAdded:Connect(function(char)
            ...
            for _, e in ipairs(Lighting2:GetChildren()) do
            ...
            for _, d in ipairs(char:GetDescendants()) do _FPS.stripVisuals(d) end
            _fpsRemoveMeshes(char)
            for _, child in ipairs(char:GetChildren()) do _fpsRemoveMeshes(child) end
        end)
    end
    _FPS.disable = function()
        for _, conn in pairs(_FPS.connections) do
            if conn then conn:Disconnect() end
        end
        _FPS.connections = {}
    end

    local _fpsOptiConn = nil

    local function enableFpsBoost()
        _G._FH_AlwaysOnFPS = true
        _FPS.enable()
        task.spawn(function() _AnimRemove.enable() end)
        pcall(function() settings().Rendering.QualityLevel = Enum.QualityLevel.Level01 end)
        local Lighting = game:GetService("Lighting")
        pcall(function()
            Lighting.GlobalShadows = false
        ...
        if _fpsOptiConn then pcall(function() _fpsOptiConn:Disconnect() end) end
        _fpsOptiConn = workspace.DescendantAdded:Connect(function(obj)
            if obj:IsA("ParticleEmitter") or obj:IsA("Trail")
            or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") then
                pcall(function() obj.Enabled = false end)
            end
    ...
    local function disableFpsBoost()
        _G._FH_AlwaysOnFPS = false
        if _fpsOptiConn then pcall(function() _fpsOptiConn:Disconnect() end); _fpsOptiConn = nil end
        _FPS.disable()
        _AnimRemove.disable()
        local Lighting = game:GetService("Lighting")
        pcall(function()
...
-- ─── Colors tab ────────────────────────────────────────────────────────────
do -- Colors tab: HUB PVP dual-accent gradient system
    local _UIS = game:GetService("UserInputService")

    local function _Corner(p, r) local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0, r or 8); c.Parent = p; return c end
    local function _Stroke(p, col, th) local s = Instance.new("UIStroke"); s.Color = col or Color3.fromRGB(45,45,45); s.Thickness = th or 1; s.Parent = p; return s end
    local function _Label(p, txt, sz, col, font) local l = Instance.new("TextLabel"); l.BackgroundTransparency = 1; l.Text = txt; l.TextSize = sz or 12; l.TextColor3 = col or Color3.fromRGB(245,245,245); l.Font = font or Enum.Font.Gotham; l.Parent = p; return l end
    local function _hsvToColor(h,s,v) return Color3.fromHSV(h%1, math.clamp(s,0,1), math.clamp(v,0,1)) end
    local function _rgbToHsv(c)
        local r,g,b = c.R,c.G,c.B; local mx,mn = math.max(r,g,b),math.min(r,g,b); local d = mx-mn; local h
        if d==0 then h=0 elseif mx==r then h=((g-b)/d)%6 elseif mx==g then h=(b-r)/d+2 else h=(r-g)/d+4 end
        h=h/6; if h<0 then h=h+1 end; return h, (mx==0 and 0 or d/mx), mx
    end

    -- Initialiser AccentA/B depuis la sauvegarde
    do
        local function _loadColor(rk, gk, bk, dr, dg, db)
            local r,g,b = SettingsObj:GetAttribute(rk), SettingsObj:GetAttribute(gk), SettingsObj:GetAttribute(bk)
            if type(r)=="number" and type(g)=="number" and type(b)=="number" then return Color3.fromRGB(r,g,b) end
            return Color3.fromRGB(dr, dg, db)
        end
        _G._FH_AccentA = _loadColor("SavedAccentA_R","SavedAccentA_G","SavedAccentA_B", 60, 210, 100)
        _G._FH_AccentB = _loadColor("SavedAccentB_R","SavedAccentB_G","SavedAccentB_B", 0, 170, 255)
        -- Reset si AccentB est encore l'ancien rose par défaut (255,120,220)
        local bR = SettingsObj:GetAttribute("SavedAccentB_R")
        local bG = SettingsObj:GetAttribute("SavedAccentB_G")
        local bB = SettingsObj:GetAttribute("SavedAccentB_B")
        if type(bR)=="number" and bR > 200 and type(bG)=="number" and bG < 140 and type(bB)=="number" and bB > 180 then
            _G._FH_AccentB = Color3.fromRGB(0, 170, 255)
        end
        -- Reset si AccentA est rose aussi
        local aR = SettingsObj:GetAttribute("SavedAccentA_R")
        local aG = SettingsObj:GetAttribute("SavedAccentA_G")
        local aB = SettingsObj:GetAttribute("SavedAccentA_B")
        if type(aR)=="number" and aR > 200 and type(aG)=="number" and aG < 60 and type(aB)=="number" and aB > 130 then
            _G._FH_AccentA = Color3.fromRGB(60, 210, 100)
        end
    end
    ...
    local function _FH_BuildThemeSequence()
        ...
        local function mix(c1,c2,t) return Color3.new(c1.R+(c2.R-c1.R)*t, c1.G+(c2.G-c1.G)*t, c1.B+(c2.B-c1.B)*t) end
        local function brighten(c,a) return Color3.new(math.min(1,c.R+(1-c.R)*a), math.min(1,c.G+(1-c.G)*a), math.min(1,c.B+(1-c.B)*a)) end
        local mid = brighten(mix(A,B,0.5), 0.15)
        return ColorSequence.new({ColorSequenceKeypoint.new(0,A), ColorSequenceKeypoint.new(0.25,mid), ColorSequenceKeypoint.new(0.5,B), ColorSequenceKeypoint.new(0.75,mid), ColorSequenceKeypoint.new(1,A)})
    end
    _G._FH_BuildThemeSequence = _FH_BuildThemeSequence

    local function _FH_BuildESPSwipeSequence()
        local ac = _G._FH_AccentA or Color3.fromRGB(120,200,255); local w = Color3.fromRGB(255,255,255)
        return ColorSequence.new({ColorSequenceKeypoint.new(0,w), ColorSequenceKeypoint.new(0.42,w), ColorSequenceKeypoint.new(0.5,ac), ColorSequenceKeypoint.new(0.58,w), ColorSequenceKeypoint.new(1,w)})
    end
    _G._FH_BuildESPSwipeSequence = _FH_BuildESPSwipeSequence

    local function _FH_AddThemeStroke(stroke)
        if not stroke then return end
        local g = stroke:FindFirstChildOfClass("UIGradient") or Instance.new("UIGradient")
        g.Parent = stroke; g.Color = _FH_BuildThemeSequence(); g.Rotation = 35; g.Transparency = NumberSequence.new(0)
        table.insert(_G._FH_ThemeStrokes, g); return g
    end
    _G._FH_AddThemeStroke = _FH_AddThemeStroke

    local function _FH_AddThemeFill(frame)
        if not frame then return end
        local g = frame:FindFirstChildOfClass("UIGradient") or Instance.new("UIGradient")
        g.Parent = frame; g.Color = _FH_BuildThemeSequence(); g.Rotation = 35; g.Transparency = NumberSequence.new(0)
        table.insert(_G._FH_ThemeFills, g); return g
    end
    _G._FH_AddThemeFill = _FH_AddThemeFill

    local function _FH_TintGUI()
        local A, B = _G._FH_AccentA, _G._FH_AccentB
        if not (A and B) then return end
        local midR, midG, midB = (A.R+B.R)*0.5, (A.G+B.G)*0.5, (A.B+B.B)*0.5
        local AMT = 0.18
        local function tintGui(sg)
                    ...
                    or d:IsA("ScrollingFrame") or d:IsA("TextBox") then
                        ...
                        if not orig then
                            ...
                            if math.max(c.R,c.G,c.B) < 0.27 and d.BackgroundTransparency < 0.98 then
                                d:SetAttribute("_FH_OrigBG", c); orig = c
                            end
                        end
                        if orig then
        ...
        local cg = game:GetService("CoreGui")
        local NAMES = {SettingsPanel=true, KeybindPanel=true, TwayveHub=true, StealBarGui=true, PanelViewUI=true, StatusDisplay=true, ActivityFeed=true, HubAdminControl=true, HubAdminPanel=true, InvisStealPanel=true}
        for _, parent in ipairs({pg, cg}) do
            if parent then
                for _, sg in ipairs(parent:GetChildren()) do
                    if NAMES[sg.Name] then pcall(tintGui, sg) end
                end
            end
        end
    end
    ...
    -- Remplace tous les éléments rose/magenta hardcodé dans tous les GUIs
    local function _recolorPink(ac)
        local function isPink(c)
            -- Catch: 255,40,160 et 255,0,220 et variantes (R élevé, G bas, B moyen-haut)
            return c.R*255 > 200 and c.G*255 < 60 and c.B*255 > 130
        end
        local pg = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")
        local cg = game:GetService("CoreGui")
        for _, parent in ipairs({pg, cg}) do
            ...
            for _, sg in ipairs(parent:GetChildren()) do
                pcall(function()
                    for _, obj in ipairs(sg:GetDescendants()) do
                        pcall(function()
                            if obj:IsA("UIStroke") and isPink(obj.Color) then
                                obj.Color = ac
                            end
                            if (obj:IsA("Frame") or obj:IsA("TextButton") or obj:IsA("ImageButton") or obj:IsA("ScrollingFrame")) then
                                if isPink(obj.BackgroundColor3) and obj.BackgroundTransparency < 0.9 then
                                    obj.BackgroundColor3 = ac
                                end
                            end
                            if (obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox")) and isPink(obj.TextColor3) then
                                obj.TextColor3 = ac
                            end
                            if (obj:IsA("ImageLabel") or obj:IsA("ImageButton")) and isPink(obj.ImageColor3) then
                                obj.ImageColor3 = ac
                            end
                        end)
                    end
    ...
    _G._recolorPink = _recolorPink

    local function _FH_UpdateThemeColors()
        local seq = _FH_BuildThemeSequence(); local espSeq = _FH_BuildESPSwipeSequence()
        ...
        _G._hubThemeColor = _G._FH_AccentA
        if _G._hubPanelStrokes then for _,st in ipairs(_G._hubPanelStrokes) do pcall(function() st.Color = _G._FH_AccentA end) end end
        pcall(_FH_TintGUI)
        pcall(_recolorPink, _G._FH_AccentA)
        pcall(function()
            SettingsObj:SetAttribute("SavedAccentA_R", math.floor(_G._FH_AccentA.R*255+0.5))
            SettingsObj:SetAttribute("SavedAccentA_G", math.floor(_G._FH_AccentA.G*255+0.5))
            SettingsObj:SetAttribute("SavedAccentA_B", math.floor(_G._FH_AccentA.B*255+0.5))
            SettingsObj:SetAttribute("SavedAccentB_R", math.floor(_G._FH_AccentB.R*255+0.5))
            SettingsObj:SetAttribute("SavedAccentB_G", math.floor(_G._FH_AccentB.G*255+0.5))
            SettingsObj:SetAttribute("SavedAccentB_B", math.floor(_G._FH_AccentB.B*255+0.5))
        end)
    end
    _G._FH_UpdateThemeColors = _FH_UpdateThemeColors

    -- Appliquer le gradient sur les strokes existants du hub
    if _G._hubPanelStrokes then
        for _,st in ipairs(_G._hubPanelStrokes) do pcall(function() _FH_AddThemeStroke(st) end) end
    end
    _G._hubThemeColor = _G._FH_AccentA

    -- Appliquer les couleurs au démarrage + loop tint toutes les 5s
    task.defer(function()
        pcall(_FH_UpdateThemeColors)
        pcall(_FH_TintGUI)
    end)
    task.spawn(function()
        while true do task.wait(5); pcall(_FH_TintGUI) end
    end)

    -- Animated gradient rotator
    if not _G._FH_ThemeRotator_v4 then
        _G._FH_ThemeRotator_v4 = true
        local function spin(list, rot)
            local n,w = #list,0
            for r2=1,n do local g=list[r2]; if g and g.Parent then w=w+1;list[w]=g;g.Rotation=rot end end
            for i=n,w+1,-1 do list[i]=nil end
        end
        task.spawn(function()
            local rot = 0
            while true do
                local st,fi = _G._FH_ThemeStrokes, _G._FH_ThemeFills
                if (st and #st>0) or (fi and #fi>0) then
                    rot = (rot+4.8)%360
                    if st then spin(st, rot) end
                    if fi then spin(fi, (rot+60)%360) end
                    task.wait(1/15)
                else task.wait(0.5) end
            end
        end)
        task.spawn(function()
            local t=-1; local lastTick=tick(); local SPEED=0.6
            while true do
                local list = _G._FH_ESPGradients
                if list and #list>0 then
                    local now=tick(); t=t+(now-lastTick)*SPEED; lastTick=now
                    if t>1 then t=t-2 end
                    local n,w=#list,0; local vec=Vector2.new(t,0)
                    for r2=1,n do local g=list[r2]; if g and g.Parent then w=w+1;list[w]=g;g.Offset=vec end end
                    for i=n,w+1,-1 do list[i]=nil end
                    task.wait(1/30)
                else lastTick=tick(); task.wait(0.5) end
            end
        end)
    end
    ...
    if colorsContent then
        for _,ch in ipairs(colorsContent:GetChildren()) do ch:Destroy() end

        local titleLbl = Instance.new("TextLabel", colorsContent)
        titleLbl.Size=UDim2.new(1,-20,0,20); titleLbl.Position=UDim2.new(0,10,0,8); titleLbl.BackgroundTransparency=1; titleLbl.Text="ACCENT COLORS"; titleLbl.TextColor3=Color3.fromRGB(245,245,245); titleLbl.Font=Enum.Font.GothamBold; titleLbl.TextSize=13; titleLbl.TextXAlignment=Enum.TextXAlignment.Left
        local subLbl = Instance.new("TextLabel", colorsContent)
        subLbl.Size=UDim2.new(1,-20,0,14); subLbl.Position=UDim2.new(0,10,0,30); subLbl.BackgroundTransparency=1; subLbl.Text="Clique A ou B pour choisir la couleur"; subLbl.TextColor3=Color3.fromRGB(110,110,110); subLbl.Font=Enum.Font.Gotham; subLbl.TextSize=10; subLbl.TextXAlignment=Enum.TextXAlignment.Left

        local SW_W, SW_H = 80, 36
        local swA, swB
        ...
        local function _openPicker(slot)
            ...
            modal.Size=UDim2.new(0,W,0,H); modal.AnchorPoint=Vector2.new(0.5,0.5); modal.Position=UDim2.new(0.5,0,0.5,0)
            modal.BackgroundColor3=Color3.fromRGB(18,18,20); modal.BackgroundTransparency=0.05; modal.BorderSizePixel=0; modal.ZIndex=60; modal.Parent=KeybindGui
            _Corner(modal, 12); _FH_AddThemeStroke(_Stroke(modal, Color3.fromRGB(255,255,255), 1))

            local hdr = Instance.new("Frame", modal)
            hdr.Size=UDim2.new(1,0,0,30); hdr.BackgroundColor3=Color3.fromRGB(10,10,10); hdr.BorderSizePixel=0; hdr.ZIndex=61
            _Corner(hdr, 12)
            local hdrLbl = _Label(hdr, slot=="a" and "Accent A" or "Accent B", 12, Color3.fromRGB(245,245,245), Enum.Font.GothamBold)
            hdrLbl.Size=UDim2.new(1,-40,1,0); hdrLbl.Position=UDim2.new(0,12,0,0); hdrLbl.TextYAlignment=Enum.TextYAlignment.Center; hdrLbl.ZIndex=62
            local closeBtn = Instance.new("TextButton", hdr)
            closeBtn.Size=UDim2.new(0,22,0,22); closeBtn.Position=UDim2.new(1,-28,0.5,-11); closeBtn.BackgroundColor3=Color3.fromRGB(140,30,30); closeBtn.BorderSizePixel=0; closeBtn.Text="×"; closeBtn.TextSize=14; closeBtn.Font=Enum.Font.GothamBold; closeBtn.TextColor3=Color3.fromRGB(255,255,255); closeBtn.AutoButtonColor=false; closeBtn.ZIndex=62
            _Corner(closeBtn, 6)

            local startColor = (slot=="a") and _G._FH_AccentA or _G._FH_AccentB
            local curH, curS, curV = _rgbToHsv(startColor)
            local SQ, HUE_H, PADX = 170, 16, (W-170)/2

            local sv = Instance.new("Frame", modal)
            sv.Size=UDim2.new(0,SQ,0,SQ); sv.Position=UDim2.new(0,PADX,0,40); sv.BackgroundColor3=_hsvToColor(curH,1,1); sv.BorderSizePixel=0; sv.ZIndex=61; sv.Active=true
            _Corner(sv, 6)
            local wOv = Instance.new("Frame", sv); wOv.Size=UDim2.new(1,0,1,0); wOv.BackgroundColor3=Color3.fromRGB(255,255,255); wOv.BorderSizePixel=0; wOv.ZIndex=62; _Corner(wOv,6)
            do local g=Instance.new("UIGradient",wOv); g.Color=ColorSequence.new(Color3.fromRGB(255,255,255)); g.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0),NumberSequenceKeypoint.new(1,1)}) end
            local bOv = Instance.new("Frame", sv); bOv.Size=UDim2.new(1,0,1,0); bOv.BackgroundColor3=Color3.fromRGB(0,0,0); bOv.BorderSizePixel=0; bOv.ZIndex=63; _Corner(bOv,6)
            do local g=Instance.new("UIGradient",bOv); g.Color=ColorSequence.new(Color3.fromRGB(0,0,0)); g.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,1),NumberSequenceKeypoint.new(1,0)}); g.Rotation=90 end
            local svDot = Instance.new("Frame", sv)
            svDot.AnchorPoint=Vector2.new(0.5,0.5); svDot.Size=UDim2.new(0,12,0,12); svDot.BackgroundColor3=Color3.fromRGB(255,255,255); svDot.BackgroundTransparency=1; svDot.BorderSizePixel=0; svDot.ZIndex=65
            Instance.new("UICorner",svDot).CornerRadius=UDim.new(1,0); _Stroke(svDot,Color3.fromRGB(255,255,255),2)

            local hueBar = Instance.new("Frame", modal)
            hueBar.Size=UDim2.new(0,SQ,0,HUE_H); hueBar.Position=UDim2.new(0,PADX,0,40+SQ+10); hueBar.BackgroundColor3=Color3.fromRGB(255,255,255); hueBar.BorderSizePixel=0; hueBar.ZIndex=61; hueBar.Active=true
            _Corner(hueBar, HUE_H/2)
            do local g=Instance.new("UIGradient",hueBar); g.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(255,0,0)),ColorSequenceKeypoint.new(0.166,Color3.fromRGB(255,255,0)),ColorSequenceKeypoint.new(0.333,Color3.fromRGB(0,255,0)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(0,255,255)),ColorSequenceKeypoint.new(0.666,Color3.fromRGB(0,0,255)),ColorSequenceKeypoint.new(0.833,Color3.fromRGB(255,0,255)),ColorSequenceKeypoint.new(1,Color3.fromRGB(255,0,0))}) end
            local hueDot = Instance.new("Frame", hueBar)
            hueDot.AnchorPoint=Vector2.new(0.5,0.5); hueDot.Size=UDim2.new(0,4,1,4); hueDot.BackgroundColor3=Color3.fromRGB(255,255,255); hueDot.BorderSizePixel=0; hueDot.ZIndex=64
            _Stroke(hueDot,Color3.fromRGB(0,0,0),1); _Corner(hueDot,2)

            local cprev = Instance.new("Frame", modal)
            cprev.Size=UDim2.new(0,28,0,28); cprev.Position=UDim2.new(0,PADX,1,-42); cprev.BackgroundColor3=startColor; cprev.BorderSizePixel=0; cprev.ZIndex=61
            _Corner(cprev,6); _Stroke(cprev,Color3.fromRGB(45,45,45),1)
            local hexLbl = _Label(modal,"",12,Color3.fromRGB(245,245,245),Enum.Font.GothamMedium)
            hexLbl.Size=UDim2.new(0,100,0,28); hexLbl.Position=UDim2.new(0,PADX+36,1,-42); hexLbl.TextXAlignment=Enum.TextXAlignment.Left; hexLbl.TextYAlignment=Enum.TextYAlignment.Center; hexLbl.ZIndex=62

            local function refresh()
                local col = _hsvToColor(curH,curS,curV)
                sv.BackgroundColor3 = _hsvToColor(curH,1,1)
                cprev.BackgroundColor3 = col
                hexLbl.Text = string.format("#%02X%02X%02X",math.floor(col.R*255+0.5),math.floor(col.G*255+0.5),math.floor(col.B*255+0.5))
                svDot.Position = UDim2.new(curS,0,1-curV,0)
                hueDot.Position = UDim2.new(curH,0,0.5,0)
                if slot=="a" then _G._FH_AccentA=col; if swA then swA.BackgroundColor3=col end
                else              _G._FH_AccentB=col; if swB then swB.BackgroundColor3=col end end
            ...
            do -- SV square drag
                local active = false
                local function upd(inp) local abs,sz=sv.AbsolutePosition,sv.AbsoluteSize; if sz.X<=0 or sz.Y<=0 then return end; curS=math.clamp((inp.Position.X-abs.X)/sz.X,0,1); curV=1-math.clamp((inp.Position.Y-abs.Y)/sz.Y,0,1); refresh() end
                sv.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then active=true;upd(inp) end end)
                sv.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then if active then _FH_UpdateThemeColors() end;active=false end end)
                _UIS.InputChanged:Connect(function(inp) if active and (inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch) then upd(inp) end end)
            end
            do -- Hue bar drag
                local active = false
                local function upd(inp) local abs,sz=hueBar.AbsolutePosition,hueBar.AbsoluteSize; if sz.X<=0 then return end; curH=math.clamp((inp.Position.X-abs.X)/sz.X,0,1); refresh() end
                hueBar.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then active=true;upd(inp) end end)
                hueBar.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then if active then _FH_UpdateThemeColors() end;active=false end end)
                _UIS.InputChanged:Connect(function(inp) if active and (inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch) then upd(inp) end end)
            end
            do -- Modal drag
                local d,ds,ps=false,nil,nil; hdr.Active=true
                hdr.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then d=true;ds=inp.Position;ps=modal.Position end end)
                hdr.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then d=false end end)
                _UIS.InputChanged:Connect(function(inp) if d and (inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch) then local dx,dy=inp.Position.X-ds.X,inp.Position.Y-ds.Y;modal.Position=UDim2.new(ps.X.Scale,ps.X.Offset+dx,ps.Y.Scale,ps.Y.Offset+dy) end end)
            end
            closeBtn.MouseButton1Click:Connect(function() _FH_UpdateThemeColors(); pickerOpen=false; if modal and modal.Parent then modal:Destroy() end end)
        end

        local function _makeSwatch(label, xPos, getColor, onOpen)
            local sw = Instance.new("TextButton", colorsContent)
            sw.Size=UDim2.new(0,SW_W,0,SW_H); sw.Position=UDim2.new(0,xPos,0,56); sw.BackgroundColor3=getColor(); sw.BorderSizePixel=0; sw.Text=label; sw.TextSize=11; sw.Font=Enum.Font.GothamBold; sw.TextColor3=Color3.fromRGB(20,20,20); sw.AutoButtonColor=false; sw.ZIndex=3
            _Corner(sw, 8); _FH_AddThemeStroke(_Stroke(sw, Color3.fromRGB(255,255,255), 1))
            sw.MouseButton1Click:Connect(onOpen)
            return sw
        end
        swA = _makeSwatch("A", 10,          function() return _G._FH_AccentA end, function() _openPicker("a") end)
        swB = _makeSwatch("B", 10+SW_W+10,  function() return _G._FH_AccentB end, function() _openPicker("b") end)

        local previewBar = Instance.new("Frame", colorsContent)
        previewBar.Size=UDim2.new(1,-20,0,24); previewBar.Position=UDim2.new(0,10,0,100); previewBar.BackgroundColor3=Color3.fromRGB(255,255,255); previewBar.BorderSizePixel=0
        _Corner(previewBar, 8); _FH_AddThemeFill(previewBar)
    end

    -- (remplace l'ancien bloc Colors tab)
    if false then
    ...
    end -- if false (ancien système désactivé)
end -- Colors tab
...
UserInputService.InputBegan:Connect(function(input, gp)
	...
	if Enum.KeyCode[_G.ResetKeybind or "X"] and input.KeyCode == Enum.KeyCode[_G.ResetKeybind or "X"] then
...
local function handleCmdLabel(label)
	...
	if direction and username and command then
		...
		-- incoming + outgoing : laisser le texte vert du jeu s'afficher normalement
	end
end
...
task.spawn(function()
    ...
    local spdSlider = makeISSlider("Spd", 5, 23, Config.StealSpeed or SharedState.GetStealSpeed(), false, function(v)
...
do
	...
	local function executeReset()
		...
		pcall(function() _Players.RespawnTime = 0 end)
		pcall(function() game:GetService("Players").RespawnTime = 0 end)
		...
		local _respawnConn
		_respawnConn = _LP.CharacterAdded:Connect(function(newChar)
			...
			task.defer(function()
				...
				_G.AntiDieDisabled = prevAntiDie
				if not prevAntiDie and _G.setupAntiDie then pcall(_G.setupAntiDie) end
				if _G._enableAntiDie then pcall(_G._enableAntiDie) end
			end)
		end)
		...
		if hum then
			...
			task.spawn(function() pcall(function() _LP:LoadCharacter() end) end)
		end
