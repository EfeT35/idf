...
do
    ...
    -- =================================================================
    -- AUTO-KICK ON STEAL  (idf hub method: shut the server down when the
    ...
    -- ============================================================
    -- ADMIN PANEL  (standalone idf hub panel -- branded idf hub, red)
...
--[[
    idf hub Admin Panel — Standalone (Fixed & Upgraded)
...
print("[idf hub Admin] script loaded — initializing…")
...
task.spawn(function()
    local ok, err = pcall(function()
    task.wait(0.15)
    print("[idf hub Admin] building UI…")
    ...
    if not highlight then
        warn("[idf hub Admin] CoreGui blocked; click-to-AP target highlight disabled")
    end
    ...
    print("[idf hub Admin] UI ready")
    end) -- end pcall

    if not ok then
        warn("[idf hub Admin] init failed: " .. tostring(err))
    end
end)
    ...
    do
        ...
        -- ══════════════════════════════════════════════════════════════════
        -- SXE AUTO GRAB ENGINE
        -- Scan tous les ProximityPrompts dans AnimalPodiums, les track,
        -- et fire en burst dès qu'on est dans le rayon du target.
        -- ══════════════════════════════════════════════════════════════════
        local SXE_FIRE_BURST    = 4
        local SXE_FIRE_DEBOUNCE = 0.12
        local SXE_ENABLE_BURST  = 4
        local SXE_RADIUS        = 60

        local sxeTracked   = {}   -- [prompt] = true
        local sxeLastFire  = {}   -- [prompt] = os.clock()
        local sxeAutoOn    = false

        local function sxeGetHRP()
            local char = LocalPlayer.Character
            return char and char:FindFirstChild("HumanoidRootPart")
        end

        local function sxeGetPromptPos(prompt)
            local p = prompt.Parent
            if not p then return nil end
            if p:IsA("Attachment") and p.Parent then p = p.Parent end
            if p:IsA("BasePart") then return p.Position
            elseif p:IsA("Model") then return p:GetPivot().Position end
        end

        local function sxeIsMyPlot(plotInst)
            if not plotInst then return false end
            local sign = plotInst:FindFirstChild("PlotSign", true)
        ...
        -- vérifie si le prompt correspond au target sélectionné (mode Highest/Priority)
        local function sxePromptMatchesTarget(prompt, targetData)
            if not targetData then return false end
            -- cherche le plot ancestor et le slot
            local plots = Workspace:FindFirstChild("Plots")
            if not plots then return false end
            local plotInst = plots:FindFirstChild(tostring(targetData.plot))
            if not plotInst then return false end
            local podiums = plotInst:FindFirstChild("AnimalPodiums")
            if not podiums then return false end
            local podium = podiums:FindFirstChild(tostring(targetData.slot))
            if not podium then return false end
            return prompt:IsDescendantOf(podium)
        end

        local function sxeCanFire(prompt)
            local now = os.clock()
            local last = sxeLastFire[prompt]
            if last and (now - last) < SXE_FIRE_DEBOUNCE then return false end
            sxeLastFire[prompt] = now
            return true
        end

        local function sxeFirePrompt(prompt, burst)
            if not prompt or not prompt.Parent then return end
            if not sxeCanFire(prompt) then return end
            if typeof(fireproximityprompt) ~= "function" then return end
            for i = 1, (burst or SXE_FIRE_BURST) do
                pcall(fireproximityprompt, prompt, 0)
            end
        end

        local function sxeIsPromptAvailable(prompt, hrpPos, targetData)
            if not prompt or not prompt.Parent then return false end
            local pos = sxeGetPromptPos(prompt)
            if not pos then return false end
            -- ignorer les plots du joueur
            local plots = Workspace:FindFirstChild("Plots")
            if plots then
                for _, plot in ipairs(plots:GetChildren()) do
            ...
            -- mode Nearest: fire tout ce qui est dans le rayon
            if Config.StealNearest or _G.MeerkoStealMode == "nearest" then
                return (pos - hrpPos).Magnitude <= SXE_RADIUS
            end
            -- mode Highest/Priority/Target manuel: matcher le target sélectionné
            return sxePromptMatchesTarget(prompt, targetData)
                and (pos - hrpPos).Magnitude <= SXE_RADIUS
        end

        local function sxeTrackPrompt(prompt)
            if sxeTracked[prompt] then return end
            sxeTracked[prompt] = true
            local function tryFire()
                if not sxeAutoOn then return end
                local hrp = sxeGetHRP()
                if not hrp then return end
                if sxeIsPromptAvailable(prompt, hrp.Position, _G.MeerkoCurrentSteal) then
                    sxeFirePrompt(prompt, SXE_ENABLE_BURST)
                end
            end
            task.defer(tryFire)
            pcall(function()
                prompt:GetPropertyChangedSignal("Enabled"):Connect(function()
                    if prompt.Enabled then task.defer(tryFire) end
                end)
            end)
            prompt.AncestryChanged:Connect(function()
                if not prompt:IsDescendantOf(Workspace) then
                    sxeTracked[prompt] = nil
                    sxeLastFire[prompt] = nil
                end
            end)
        end

        local function sxeScanAll()
            local plots = Workspace:FindFirstChild("Plots")
        ...
        Workspace.DescendantAdded:Connect(function(obj)
            if obj:IsA("ProximityPrompt") then
                local ok = obj:FindFirstAncestor("AnimalPodiums")
                if ok then sxeTrackPrompt(obj) end
            end
        end)
        ...
        local function _onCharAdded(char)
            char.ChildAdded:Connect(function(obj)
                if _isBrainrotTool(obj) then
                    _holdingBrainrot = true; _firstGrabDone = true
                end
            end)
        ...
        -- ── SXE poll loop ─────────────────────────────────────────────
        local _stealReadyAt = os.clock() + 3
        local _petsCacheT   = 0
        local _petsCache    = {}
        ...
        task.spawn(function()
            while true do
                ...
                local active = Config.StealNearest or Config.StealHighest or Config.StealPriority
                    or _G.MeerkoStealMode ~= nil or _G.MeerkoStealTargetUID ~= nil
                if not _firstGrabDone and not active then
                    sxeAutoOn = false; continue
                end
                if not active then
                    sxeAutoOn = false
                    if barActive then hideBar() end
                    currentTargetUID = nil; _G.MeerkoCurrentSteal = nil
                    continue
                end

                -- refresh la liste des pets ~1x/sec pour Highest/Priority
                local now = os.clock()
                ...
                -- choisir le target courant (pour le bar + la sélection de prompt)
                local tp = nil
                if not Config.StealNearest and _G.MeerkoStealMode ~= "nearest" then
                    tp = pickTarget(_petsCache)
                    if tp then
                ...
                local hrp = sxeGetHRP()
                if not hrp then sxeAutoOn = false; continue end
                local myPos = hrp.Position

                -- fire tous les prompts disponibles (SXE style burst)
                local anyFired = false
                for prompt in pairs(sxeTracked) do
                    if sxeIsPromptAvailable(prompt, myPos, tp) then
                        sxeAutoOn = true
                        sxeFirePrompt(prompt, SXE_FIRE_BURST)
                        anyFired = true
                        -- bar UI
                        local name = tp and tp.name or "brainrot"
                        if not barActive then
                            barLabel.Text = "Stealing " .. name .. "..."
                            barActive = true
                        end
                    end
                end
    ...
    -- =================================================================
    -- X-RAY BASES  (ported from idf hub): make base walls translucent via
    ...
    task.spawn(function()
        ...
        -- ===== idf hub header badge (compact, menu-themed) =====
        local TweenService = game:GetService("TweenService")
        ...
        brand.Font = Enum.Font.GothamBold; brand.Text = "idf hub"
    ...
    task.spawn(function()
        ...
        -- ============================================================
        -- AUTO TURRET
        -- Détecte les Sentry_ ennemis, les téléporte devant le joueur
        -- et les bat avec le Bat. Gated by Config.AutoDestroyTurrets.
        -- ============================================================
        do
            local _atActive  = false
            local _atConn    = nil
            local _prevTool  = nil
            local _atRun     = game:GetService("RunService")

            local function _atGetHRP()
                local char = LocalPlayer.Character
                return char and char:FindFirstChild("HumanoidRootPart")
            end
            local function _atGetHum()
                local char = LocalPlayer.Character
                return char and char:FindFirstChildOfClass("Humanoid")
            end

            local function _atFindSentries()
                local hrp = _atGetHRP()
                if not hrp then return {} end
                local out, myId = {}, tostring(LocalPlayer.UserId)
                for _, d in ipairs(Workspace:GetDescendants()) do
                    if d and d.Parent and d.Name and d.Name:sub(1,7) == "Sentry_" then
                        local ownerId = d.Name:match("Sentry_(.+)")
                        if ownerId and ownerId ~= myId and ownerId ~= LocalPlayer.Name then
                            local part = d:IsA("BasePart") and d or d:FindFirstChildWhichIsA("BasePart", true)
                            if part then
                                table.insert(out, { obj = d, part = part, dist = (hrp.Position - part.Position).Magnitude })
                            end
                        end
                    end
                end
            ...
            local function _atEquipBat()
                local char = LocalPlayer.Character
                local hum  = _atGetHum()
                if not char or not hum then return nil end
                local cur = char:FindFirstChildWhichIsA("Tool")
                if cur and cur.Name ~= "Bat" then _prevTool = cur end
            ...
            local function _atRestoreTool()
                pcall(function()
                    local char = LocalPlayer.Character
                    local hum  = _atGetHum()
                    if not char or not hum then return end
                    local bat = char:FindFirstChild("Bat")
                    if bat then hum:UnequipTools() end
                    if _prevTool and _prevTool.Parent then
                        hum:EquipTool(_prevTool)
                    end
                end)
                _prevTool = nil
            end

            local function _atNukeSentry(obj)
                pcall(function()
            ...
            local function startAutoTurretLoop()
                if _atConn then return end
                _atActive = true
                _G.autoTurretActive = true
                local frameSkip = 0
                local currentTarget = nil
                local attackCd = 0
                _atConn = _atRun.Heartbeat:Connect(function()
                    if not _atActive then return end
                    if LocalPlayer:GetAttribute("Stealing") == true then
                        currentTarget = nil; return
                    end
                    frameSkip = frameSkip + 1
                    if frameSkip < 4 then return end
                    frameSkip = 0

                    -- refresh target if lost
                    if not currentTarget or not currentTarget.obj.Parent then
                        local list = _atFindSentries()
                    ...
                    -- move sentry in front and make it hittable
                    local targetCF = hrp.CFrame * CFrame.new(0, 0, -3)
                    pcall(function()
                        _atNukeSentry(currentTarget.obj)
                        if currentTarget.obj:IsA("Model") and currentTarget.obj.PrimaryPart then
                            currentTarget.obj:SetPrimaryPartCFrame(targetCF)
                        elseif currentTarget.part then
                            currentTarget.part.CFrame = targetCF
                        end
                    end)

                    -- equip bat and hit
                    local now = tick()
                    if now - attackCd < 0.05 then return end
                    attackCd = now
                    pcall(function()
                        local char = LocalPlayer.Character
                        if not char then return end
            ...
            local function stopAutoTurretLoop()
                ...
                if _atConn then _atConn:Disconnect(); _atConn = nil end
                task.spawn(_atRestoreTool)
            end
    ...
    task.spawn(function()
        ...
        local FG_Theme = {
            Window   = rgb(255, 252, 255),
            WindowB  = rgb(255, 249, 252),
            Sidebar  = rgb(252, 245, 249),
            TopBar   = rgb(255, 249, 252),
            Card     = rgb(255, 255, 255),
            CardAlt  = rgb(250, 242, 248),
            CardAlt2 = rgb(244, 230, 240),
            Stroke   = rgb(238, 188, 219),
            StrokeS  = rgb(220, 150, 195),
            Text     = rgb(40, 15, 30),
            SubText  = rgb(140, 80, 115),
            Faint    = rgb(190, 140, 170),
            Accent   = rgb(232, 111, 177),
            Accent2  = rgb(195, 65, 140),
            AccentSoft = rgb(252, 225, 240),
            Green    = rgb(232, 111, 177),
            Green2   = rgb(210, 80, 155),
            Track    = rgb(243, 204, 228),
            Shadow   = rgb(200, 100, 160),
        }
        ...
        function Section:AddDivider()
            ...
            fg_new("Frame", {
                ...
                BackgroundColor3 = Color3.fromRGB(232, 111, 177),
        ...
        -- ============================================================
        -- Build the new idf hub window
        -- ============================================================
        local Window = Library:CreateWindow({ Title = "idf hub" })
